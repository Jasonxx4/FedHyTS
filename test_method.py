...
def test_spa_model(s_emb, label, device):
    ...
    for idx, la in tqdm(enumerate(label)):
        recall[idx, 0] += len(list(set(label_r[idx, :10]).intersection(set(la[:10]))))  
        recall[idx, 1] += len(list(set(label_r[idx, :50]).intersection(set(la[:50]))))  
        recall[idx, 2] += len(list(set(label_r[idx, :50]).intersection(set(la[:10]))))  
        recall[idx, 3] += len(list(set(label_r[idx, :1]).intersection(set(la[:1]))))  
        recall[idx, 4] += len(list(set(label_r[idx, :10]).intersection(set(la[:1]))))  
        recall[idx, 5] += len(list(set(label_r[idx, :50]).intersection(set(la[:1]))))  
    return recall
